...
async function fetchPOSTrequestFromCorrectEndpoint(
  ...
  if (selectedTeam === "" && selectedCustomer === "") {
    response = null;
    setRowResultsOfDB(fullRowResultsOfDBinCache); // setting = to cached state here, no endpoint accessed
  } else
    response = await axios.post("/api/getTeamAndCustomerSelectionsFiltered", {
      selectedTeam,
      selectedCustomer,
  ...
  // const [columnHeadToSort, order] = ["id", "Ascending"]; // DUMMY DATA
